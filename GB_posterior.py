...
def main(argv=None):
    ...
    batch_indexes = args.batch_index.split('-')
    if len(batch_indexes) == 1:
        batch_index_list = [int(batch_indexes[0])]
    else:
        batch_index_list = list(range(int(batch_indexes[0]), int(batch_indexes[1])+1))
    with open('globalGB/GB_search_config.json', 'r') as f:
        config = json.load(f)
        config = GBConfig(config)
    for batch_index in batch_index_list:
        print(f"Processing batch index {batch_index}")
